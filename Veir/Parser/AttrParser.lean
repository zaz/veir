...
def parseOptionalNumberAttr : AttrParserM (Option Attribute) := do
  ...
  -- The type is a builtin integer or float type, or a `!name` alias standing
  -- for one.
  let mut integerType? ← parseOptionalIntegerType
  let mut floatType? ← if integerType?.isSome then pure none else parseOptionalFloatType
  if integerType?.isNone && floatType?.isNone then
    if let some name ← parseOptionalPrefixedKeyword .exclamationIdent then
      if let some alias ← resolveOptionalTypeAlias typePos name then
        integerType? := alias.cast? IntegerType
        floatType? := alias.cast? FloatType
  ...
  let some floatType := floatType?
    | throwAt typePos "integer type expected after ':' in integer attribute"
